...
//                     spacepos[0],spacepos[1],spacepos[2],spacepos[3],spacepos[4],spacepos[5],vel);
    ...
    targetp_[0]=int((targetp[0]*(10000.0*mp[0].transmission) + mp[0].initDec)
            *mp[0].direction);
    targetp_[1]=int((targetp[1]*(10000.0*mp[1].transmission) + mp[1].initDec)
            *mp[1].direction);
    targetp_[2]=int((targetp[2]*(10000.0*mp[2].transmission) + mp[2].initDec)
            *mp[2].direction);
    targetp_[3]=int((targetp[3]*(10000.0*mp[3].transmission) + mp[3].initDec)
            *mp[3].direction);
    targetp_[4]=int((targetp[4]*(10000.0*mp[4].transmission) + mp[4].initDec)
            *mp[4].direction);
...
int filterMaxVelocity(double maxvelocity,int acctime,int dectime,int v,int targetdec,int *vel)
{
    ...
    int planDistance = abs(double(velMax - maxvelocity)/2000.0 * double(acctime)
            + double(maxvelocity) / 1000.0 * double(acctime) )
            + abs(double(velMax) / 2000.0 * double(dectime));

    if(planDistance > targetdec){
        if(velMax<0)
        {
            velMax = -(double(targetdec) - double(maxvelocity) / 2000.0 * double(acctime))
                    /((double(acctime) + double(dectime))/2000.0);
        }else{
            velMax = (double(targetdec) - double(maxvelocity) / 2000.0 * double(acctime))
                    /((double(acctime) + double(dectime))/2000.0);
        }
    ...
    int accDistance = abs(double(velMax - maxvelocity)/2000.0 * double(acctime)
            + double(maxvelocity) / 1000.0 * double(acctime));
