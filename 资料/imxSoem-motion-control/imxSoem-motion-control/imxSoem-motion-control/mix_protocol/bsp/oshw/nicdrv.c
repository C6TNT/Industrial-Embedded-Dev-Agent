 ...
 * configurations. */
const uint16 priMAC[3] = {0x54, 0x27, 0x8d};
/** Secondary source MAC address used for EtherCAT. */
const uint16 secMAC[3] = {0x00, 0x00, 0x00};
 ...
 */
int ecx_setupnic(ecx_portt *port, enet_handle_t handle, const char *ifname, int secondary)
{
   int i;
   enet_handle_t *psock;
   if (secondary)
   {
      ...
      if (port->redport)
      {
         ...
         memset(psock, 0, sizeof(enet_handle_t));
   ...
   {
      memset(&(port->sockhandle), 0, sizeof(enet_handle_t));
      port->lastidx = 0;
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   // rval=ENET_SendFrame(ENET1,stack->sock, (*stack->txbuf)[idx], lp,0, false, NULL);
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   if (port->redstate != ECT_RED_NONE)
   {
      ...
      // EthWrPacket(&(port->txbuf2), port->txEC_PRINTFbuflength2);
      // ENET_SendFrame(ENET1,stack->sock, port->txbuf2, port->txbuflength2,0, false, NULL );
      // port->redport->rxbufstat[idx] = EC_BUF_TX;
      rval = ENET_SendFrame(ENET1, &(port->redport->sockhandle), port->txbuf2, port->txbuflength2, 0, false, NULL);

      if (!rval)
      {
         port->redport->rxbufstat[idx] = EC_BUF_TX;
         return port->txbuflength2;
      }
      else
      {
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   status = ENET_GetRxFrameSize(stack->sock, &bytesrx, 0);
   if ((status == kStatus_Success) && (bytesrx != 0))
   {
      ENET_ReadFrame(ENET1, stack->sock, (*stack->tempbuf), bytesrx, 0, NULL);
      port->tempinbufs = bytesrx;
   }
   else if (status == kStatus_ENET_RxFrameError)
   {
      /* update the receive buffer. */
      ENET_ReadFrame(ENET1, stack->sock, NULL, 0, 0, NULL);
      port->tempinbufs = 0;
   }
   else
   {
...
#ifdef EC_VER1
int ec_setupnic(const char *ifname, enet_handle_t handle, int secondary)
{
   return ecx_setupnic(&ecx_port, handle, ifname, secondary);
}
