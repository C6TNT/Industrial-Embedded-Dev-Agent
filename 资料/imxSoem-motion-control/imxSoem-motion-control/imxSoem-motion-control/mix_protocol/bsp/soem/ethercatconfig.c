...
int ecx_config_init(ecx_contextt *context, uint8 usetable)
{
   ...
   if (wkc > 0)
   {
      PRINTF("===== Scanned %d slaves, low-level information as follows =====\r\n", *(context->slavecount));
      for (int slave = 1; slave <= *(context->slavecount); slave++)
      {
         PRINTF("----- Slave %d -----\r\n", slave);
         PRINTF("Config address: 0x%04X, Alias address: 0x%04X\r\n", context->slavelist[slave].configadr, context->slavelist[slave].aliasadr);
         PRINTF("Vendor ID: 0x%08X, Product ID: 0x%08X, Revision: 0x%08X\r\n", context->slavelist[slave].eep_man, context->slavelist[slave].eep_id, context->slavelist[slave].eep_rev);
         PRINTF("DC sync supported: %s\r\n", context->slavelist[slave].hasdc ? "Yes" : "No");
         PRINTF("Slave name: %s\r\n", context->slavelist[slave].name);
         PRINTF("Active ports: 0x%02X, Topology port count: %d\r\n", context->slavelist[slave].activeports, context->slavelist[slave].topology);
         PRINTF("Mailbox config: Write addr 0x%04X (length 0x%04X), Read addr 0x%04X (length 0x%04X)\r\n",
                context->slavelist[slave].mbx_wo, context->slavelist[slave].mbx_l,
                context->slavelist[slave].mbx_ro, context->slavelist[slave].mbx_rl);
      }
   }
   return wkc;
}
