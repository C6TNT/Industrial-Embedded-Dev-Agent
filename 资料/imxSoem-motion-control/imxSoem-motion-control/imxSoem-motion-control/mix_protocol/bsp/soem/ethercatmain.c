 ...
 * @return >0 if OK
 */
int ecx_init(ecx_contextt *context, enet_handle_t handle,const char * ifname)
{
   return ecx_setupnic(context->port, handle, ifname, FALSE);
}
 ...
 * @return >0 if OK
 */
int ecx_init_redundant(ecx_contextt *context, ecx_redportt *redport, enet_handle_t handle,const char *ifname, char *if2name)
{
   int rval, zbuf;
   ec_etherheadert *ehp;

   context->port->redport = redport;
   ecx_setupnic(context->port, handle,ifname, FALSE);
   rval = ecx_setupnic(context->port,handle,if2name, TRUE);
 ...
 * @see ecx_init
 */
int ec_init(const char * ifname, enet_handle_t g_handle)
{
   return ecx_init(&ecx_context, g_handle, ifname);
}
 ...
 */
int ec_init_redundant(enet_handle_t handle,const char *ifname, char *if2name)
{
   return ecx_init_redundant (&ecx_context, &ecx_redport, handle,ifname, if2name);
}
